...
namespace HTC.UnityPlugin.Pointer3D
{
    ...
    {
        private static List<RaycastResult> hits;

        public RaycastResult[] HitResults { get { return hits.ToArray(); } }
        ...
        public override int Raycast(Ray ray, float distance, List<RaycastResult> raycastResults)
        {
            ...
            for (int i = tempCanvases.Count - 1; i >= 0; --i)
            {
                ...
                hitCount += Raycast(target.canvas, target.ignoreReversedGraphics, ray, distance, raycaster, raycastResults);
                hits.AddRange(raycastResults);
            }
            ListPool<ICanvasRaycastTarget>.Release(tempCanvases);
            return hitCount;
        }

        public static int Raycast(Canvas canvas, bool ignoreReversedGraphics, Ray ray, float distance, Pointer3DRaycaster raycaster, List<RaycastResult> raycastResults)
        {
            int counts = 0;
            hits = new List<RaycastResult>();

            if (canvas == null) { return counts; }
